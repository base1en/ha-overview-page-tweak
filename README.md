...
[Home Assistant Builder (hab)](https://github.com/home-assistant/hab) is a CLI
utility designed for LLMs to build and manage Home Assistant configurations,
built and released by Paulus Schoutsen.

At the same time, the Home Dashboard is a strategy dashboard with no card
slots, so a `button-card` or similar cannot be placed into its header through
Lovelace.
...
> On HA reload or system boot the old view remains until this component is injected.

> Wait until HA has finished loading, then either refresh the page or switch to another
...
2. Load it app-wide via `frontend.extra\_module\_url` in `configuration.yaml`:
...
`frontend.extra\_module\_url`. After that, editing the JS itself needs only a
...
> `extra\_module\_url` injects the script on every page, but the module only
...
* **`frontend.extra\_module\_url` is a supported configuration option for
...
|-|-|-|
|Title not changed|Module not loaded, or route not in scope|Hard-refresh; verify URL in `extra\_module\_url`; confirm you are on `/home`|
